...
<ul id="search-menu" class="cw-list-searchoptions">
	<%using (Html.BeginForm(MVC.Search.Results(), FormMethod.Get, new { id = "searchForm"})) { %>
		...
		var searchGroups = from p in Model.SearchMenuProperties
		...
		<%
			var p = grp.SearchProperties.First();
			var props = p.SearchTypeId == (int)SearchTypes.Contains ? 
				grp.SearchProperties.Where(x => x.PropertyId == p.PropertyId) :
				grp.SearchProperties;

			var isGrouping = grp.SearchProperties.Count() > 1;
		
		//	var prop = grp.SearchProperties.First();
			...
			foreach(var prop in props){
				...
				var valueClass = ""; 
				  ...
				  %>
				<li>
				<%if (searchType.Equals(SearchTypes.Contains) || searchType.Equals(SearchTypes.Range)) {//!isGroup && (searchType == SearchTypes.Contains || searchType == SearchTypes.Range || searchType == SearchTypes.TagText)) { %>
				<label><%: isGrouping ? prop.SearchGroup : prop.DisplayName%></label>
				<%} %>
